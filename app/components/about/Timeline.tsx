          ...
          <Reveal as="li" key={`${entry.year}-${entry.title}`} delay={(i % 3) * 0.05} y={22}>
            <div className="grid grid-cols-1 gap-4 py-7 md:grid-cols-6 md:gap-8">
              <div className="flex items-center gap-4 md:col-span-1 md:justify-end md:pr-8">
                <span className="mono order-2 text-[0.72rem] text-brass md:order-1">
                  {entry.year}
                </span>
                <span
                  className="order-1 h-[15px] w-[15px] shrink-0 rotate-45 border md:order-2 md:-mr-[calc(2rem+7px)]"
                  style={{
                    borderColor: entry.kind === 'award' ? 'var(--brass)' : 'var(--line-3)',
                    background: entry.kind === 'award' ? 'var(--brass)' : 'var(--ink)',
                  }}
                />
              </div>

              <div className="pl-8 md:col-span-5 md:pl-0">
                <p className="eyebrow mb-2">{kindLabels[entry.kind]}</p>
                <h3 className="display-tight text-[clamp(1.15rem,2.1vw,1.6rem)] text-bone">
                  {entry.title}
                </h3>
                {entry.org ? <p className="mt-1 text-sm text-brass">{entry.org}</p> : null}
                <p className="mt-3 max-w-2xl text-sm leading-relaxed text-bone-2">{entry.body}</p>
                {entry.tags ? (
